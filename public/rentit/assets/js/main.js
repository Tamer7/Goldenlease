    ...
    *  Form boging chnaged
    ...
    $(".PickingUpDate").datepicker({
        ...
        onSelect: function (dateText, inst) {
            formBookingChanged();
            var pickupDate = new Date(dateText);
            var droppingOffDate = new Date(pickupDate);
            droppingOffDate.setDate(pickupDate.getDate() + 3);

            $(".DroppingOffDate").removeAttr("disabled");
            $(".DroppingOffDate").datepicker("option", "minDate", droppingOffDate);
            $(".DroppingOffDate").datepicker("setDate", droppingOffDate);
            $(".DroppingOffDate").val($.datepicker.formatDate("mm/dd/yy", droppingOffDate));

            formBookingChanged();
            // $(".DroppingOffDate").datepicker("option", "minDate", new Date(dateText));
            // $(".DroppingOffDate").val($.datepicker.formatDate("mm/dd/yy", new Date(dateText)));
        }
    });

    // Logic to get pickup date from session value if exists 
    let dropOffDateToSet = null;
    let sessionPickupDate = $('.PickingUpDate').val();
    if (sessionPickupDate) {
        let pickupDate = new Date(sessionPickupDate);
        dropOffDateToSet = new Date(pickupDate);
        dropOffDateToSet.setDate(pickupDate.getDate() + 3);
    }


    $('.DroppingOffDate').datepicker({
        minDate: dropOffDateToSet ?? today,
        format: rentit_obj.date_format,
        locale: rentit_obj.lang,
        disabledDates: rentit_obj.reserved_date,
        onSelect: function (dateText, inst) {
    ...
    // jQuery(".PickingUpDate").on("select", function (e) {
    //     formBookingChanged();
    //     jQuery(".DroppingOffDate").data("datepicker").minDate(e.date);
    //     jQuery(".DroppingOffDate").val(moment(e.date).format(rentit_obj.date_format));
    // });
    // jQuery(".DroppingOffDate").on("select", function (e) {
    //     formBookingChanged();
    // });
